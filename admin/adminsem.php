...
$ENCID = $_GET['id'];
$dept_id= encryptor('d', base64_decode($ENCID));

$sql = "SELECT * FROM `sem` WHERE 1";
...
?>
      ...
      <!-- BEGIN : MAIN CONTENT -->
      <h1 class=" my-6 mx-auto px-4 md:px-12 text-3xl font-extrabold tracking-tight text-gray-900 dark:text-white">Semester List</h1>

      <!-- <div class="container my-12 mx-auto px-4 md:px-12"> -->
    <!-- <div class="flex flex-wrap text-3xl lg:-mx-4"> -->
    <?php
     if (mysqli_num_rows($result) > 0 ) {
      while($row = mysqli_fetch_assoc($result)) {
        $sem_id = $row['id'];
        $sql1 = "SELECT `id`FROM `ds` WHERE dept_id = $dept_id AND sem_id = $sem_id";
        $result1 = mysqli_query($conn, $sql1);
        $row1 = mysqli_fetch_assoc($result1);
        $MAC_ID = base64_encode(encryptor('e', $row1['id']));
        ?>

    <div class="chat__chat-list overflow-y-auto scrollbar-hidden pr-1 pt-1 mt-4">
                        <div class="card intro-x mt-4 cursor-pointer box relative flex p-3 ">
                            <div class="ml-2 mt-2 overflow-hidden">
                                <a href="<?php echo "adminsubject.php?id=" . $MAC_ID ?>">
